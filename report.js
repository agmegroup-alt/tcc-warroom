...
function buildWhatsApp(wk, day, yesterday, done, person) {
  ...
  // All pending since start
  if (allPending.length > 0) {
    msg += `⚠️ *${allPending.length} TOTAL PENDING (since Week 1):*\n`;
    // Group by week and day
    let lastLabel = '';
    allPending.forEach((p, i) => {
      const label = `Wk ${p.week} · ${p.day}`;
      if (label !== lastLabel) { msg += `\n_${label}_\n`; lastLabel = label; }
      msg += `${i+1}. ${p.task}\n`;
    });
    msg += `\n`;
  } else {
    msg += `✅ All tasks completed up to today!\n\n`;
  }

  // Today's tasks
  if (today.completed.length > 0) {
    msg += `*Done today (${today.completed.length}):*\n`;
    today.completed.forEach(t => { msg += `✓ ${t}\n`; });
    msg += `\n`;
  }
  if (today.pending.length > 0) {
    msg += `*📋 Today (${day}) — ${today.pending.length} remaining:*\n`;
    today.pending.forEach((t, i) => { msg += `${i+1}. ${t}\n`; });
  } else {
    msg += `🎯 *All today\'s tasks done!*\n`;
  }
...
async function main() {
  ...
  // WhatsApp: personalized message per person
