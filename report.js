...
// Get ALL pending tasks from Week 1 Day 1 up to (but not including) today
function getAllPending(done, currentWk, currentDay, person) {
  const dayOrder = ['MON','TUE','WED','THU','FRI','SAT','SUN'];
  const currentDayIdx = dayOrder.indexOf(currentDay);
  const pending = [];

  for (let w = 1; w <= currentWk; w++) {
    const days = w < currentWk ? dayOrder : dayOrder.slice(0, currentDayIdx);
    days.forEach(day => {
      const tasks = getTasksForDay(w, person, day);
      tasks.forEach((text, i) => {
        if (!done[`${w}-${person}-${day}-${i}`]) {
          pending.push({ week: w, day, task: text });
        }
      });
    });
  }
...
function buildWhatsApp(wk, day, yesterday, done, person) {
  ...
  const allPending = getAllPending(done, wk, day, person);
  const green = GRN[wk-1] > 0 ? ` · 🌱 ${GRN[wk-1].toLocaleString()} lbs green` : '';

  const messages = [];

  // Message 1: Summary + today's tasks (kept short)
  let msg1 = `☕ *THE COFFEE CODE*\n`;
  msg1 += `Week ${wk} · ${day} · Daily Report\n`;
  msg1 += `━━━━━━━━━━━━━━━━━━━━\n\n`;
  msg1 += `*${name}* — ${prog.pct}% this week (${prog.doneCount}/${prog.total})\n`;
  msg1 += `📦 ${PKS[wk-1]} packs · 💰 $${PRF[wk-1].toLocaleString()} profit${green}\n\n`;
  msg1 += allPending.length > 0
    ? `⚠️ *${allPending.length} tasks pending since Week 1* (next message)\n\n`
    : `✅ Zero pending tasks — perfect execution!\n\n`;
  if (today.completed.length > 0) {
    msg1 += `*Done today (${today.completed.length}):*\n`;
    today.completed.forEach(t => { msg1 += `✓ ${t}\n`; });
    msg1 += `\n`;
  }
  if (today.pending.length > 0) {
    msg1 += `*📋 Today (${day}) — ${today.pending.length} to do:*\n`;
    today.pending.forEach((t, i) => { msg1 += `${i+1}. ${t}\n`; });
  } else {
    msg1 += `🎯 *All today\'s tasks done!*\n`;
  }
  msg1 += `\n👉 project-mnv5i.vercel.app`;
  messages.push(msg1);

  // Message 2+: All pending tasks chunked under 1500 chars
  if (allPending.length > 0) {
    let chunk = `⚠️ *ALL PENDING — ${name}*\n\n`;
    let lastLabel = '';
    allPending.forEach((p, i) => {
      const label = `_Wk ${p.week} · ${p.day}_\n`;
      const line  = `${i+1}. ${p.task}\n`;
      if (label !== `_${lastLabel}_\n` && lastLabel !== `Wk ${p.week} · ${p.day}`) {
        if ((chunk + label + line).length > 1450) {
          messages.push(chunk);
          chunk = `⚠️ *PENDING (cont.) — ${name}*\n\n`;
        }
        chunk += label;
        lastLabel = `Wk ${p.week} · ${p.day}`;
      }
      if ((chunk + line).length > 1450) {
        messages.push(chunk);
        chunk = `⚠️ *PENDING (cont.) — ${name}*\n\n`;
        lastLabel = '';
      }
...
function buildEmail(wk, day, yesterday, done) {
  ...
  const allPendA = getAllPending(done, wk, day, 'a');
  const allPendX = getAllPending(done, wk, day, 'x');
  ...
  const allPendRows = (items) => {
    let h = ''; let lastLabel = '';
    items.forEach(p => {
      const label = `Wk ${p.week} · ${p.day}`;
      if (label !== lastLabel) {
        h += `<li style="padding:6px 0 2px;font-size:10px;letter-spacing:1px;text-transform:uppercase;color:#9CA3AF;font-weight:600;list-style:none;border-bottom:none">${label}</li>`;
        lastLabel = label;
      }
      h += `<li style="color:#92400E;padding:4px 0;font-size:12px;border-bottom:1px solid #FEF3C7;display:flex;gap:6px;align-items:flex-start"><span style="flex-shrink:0">⚠</span><span>${p.task}</span></li>`;
    });
    return h;
  };
...
${(allPendA.length > 0 || allPendX.length > 0) ? `
<div style="background:#FFFBEB;border:1px solid #FDE68A;border-top:none;padding:16px 20px">
  <div style="font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#92400E;margin-bottom:12px">⚠️ ALL PENDING SINCE WEEK 1 — COMPLETE BEFORE MOVING FORWARD</div>
  ${allPendA.length > 0 ? `
  <div style="margin-bottom:16px">
    <div style="font-size:12px;font-weight:700;color:#78350F;margin-bottom:6px">ALEJANDRO — ${allPendA.length} total pending</div>
    <ul style="list-style:none;padding:0;margin:0">${allPendRows(allPendA)}</ul>
  </div>` : `<p style="font-size:12px;color:#15803D;margin:0 0 8px">✅ Alejandro has no pending tasks — perfect execution</p>`}
  ${allPendX.length > 0 ? `
  <div>
    <div style="font-size:12px;font-weight:700;color:#1D4ED8;margin-bottom:6px">ALEX — ${allPendX.length} total pending</div>
    <ul style="list-style:none;padding:0;margin:0">${allPendRows(allPendX)}</ul>
  </div>` : `<p style="font-size:12px;color:#15803D;margin:0">✅ Alex has no pending tasks — perfect execution</p>`}
</div>` : `
<div style="background:#F0FDF4;border:1px solid #BBF7D0;border-top:none;padding:12px 20px">
  <p style="font-size:13px;color:#15803D;font-weight:600;margin:0">✅ Zero pending tasks since Week 1. Perfect execution from both.</p>
</div>`}
...
async function main() {
  ...
  // Email: one report to agmegroup@gmail.com with both Alejandro + Alex sections
  await sendEmail('agmegroup@gmail.com', subject, html);
  console.log('✅ Email sent to agmegroup@gmail.com');

  // WhatsApp: personalized chunked messages per person
  const msgsA = buildWhatsApp(wk, day, yest, done, 'a');
  const msgsX = buildWhatsApp(wk, day, yest, done, 'x');
  for (const msg of msgsA) await sendWhatsApp(PHONE_A, msg);
  for (const msg of msgsX) await sendWhatsApp(PHONE_X, msg);
  console.log('✅ WhatsApp sent to Alejandro + Alex');
}
